...
<template>
  <div>
    ...
    <div mb-2 flex items-center>
      ...
      <n-input-number v-model:value="count" flex-1 :min="1" :max="50"
        :placeholder="$t('tools.uuid-generator.quantityPlaceholder')" />
    </div>

    <div v-if="version === 'v3' || version === 'v5'">
      <div>
        <c-buttons-select v-model:value="v35Args.namespace" :options="{
      ...
      <div flex-1>
        <c-input-text v-model:value="v35Args.namespace" :placeholder="$t('tools.uuid-generator.namespacePlaceholder')"
          label-width="100px" label-position="left" label=" " :validation-rules="validUuidRules" mb-2 />
      </div>

      <c-input-text v-model:value="v35Args.name" :placeholder="$t('tools.uuid-generator.namePlaceholder')"
        :label="$t('tools.uuid-generator.name')" label-width="100px" label-position="left" mb-2 />
    </div>

    <c-input-text style="text-align: center; font-family: monospace" :value="uuids" multiline
