...
<template>
  <div style="flex: 0 0 100%">
    <div style="max-width: 600px" :class="{ 'flex-col': styleStore.isSmallScreen }" mx-auto mb-5 flex gap-2>
      <c-select v-model:value="config.language" flex-1 :label="t('tools.sql-prettify.dialect')" :options="[
      ...
      <c-select v-model:value="config.keywordCase" :label="t('tools.sql-prettify.keywordCase')" flex-1 :options="[
      ...
      <c-select v-model:value="config.indentStyle" :label="t('tools.sql-prettify.indentStyle')" flex-1 :options="[
  ...
  <n-form-item :label="t('tools.sql-prettify.inputLabel')">
    <c-input-text ref="inputElement" v-model:value="rawSQL" :placeholder="t('tools.sql-prettify.inputPlaceholder')"
